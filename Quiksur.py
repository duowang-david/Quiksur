...
class main_menu(tk.LabelFrame):
    def __init__(self, original):
        ...
        # Create logo image widget and place it at the lower left corner
        self.img = ImageTk.PhotoImage(Image.open('logo.png'))
        self.background_image = tk.Label(original, image=self.img)
        self.background_image.grid(row=1, column=0, padx=10, pady=15, sticky='n')

    # define methods for main menu buttons
    @staticmethod
    def load_survey():
        # load survey from json file, open file dialog and let user to select a file
        load_filename = filedialog.askopenfile(initialdir='/Users',
                                               title='Select a file', filetypes=(('json files', '*.json'),
                                                                                 ('all files', '*.*')))
        if load_filename is not None:   # if user hit cancel, filedialog returns Nonetype; if it's not none, open file
            with open(load_filename.name, 'r') as in_file:  # using context manager to open file
                in_data = json.load(in_file)
                for data in in_data:
                    survey_question.append(in_data[data])   # load questions from json file to survey_question list
        page_control.update_page_status()   # show loaded question on screen

    @staticmethod
    def export_survey():    # export survey to a json file
        if len(survey_question) == 0:   # if there's no question in survey, show alert
            messagebox.showinfo('Export Survey', 'Your survey is empty!')
        else:   # open file dialog and let user to save json file
            export_filename = filedialog.asksaveasfile(initialdir='/Users',
                                                       title='Select a directory', filetypes=(('json files', '*.json'),
                                                                                              ('all files', '*.*')))
            if export_filename is not None:     # if user didn't hit cancel, the program gets a proper file name
                with open(export_filename.name, 'w') as outfile:    # open file, then write question in json format
                    out_data = dict()
    ...
    @staticmethod
    def start_survey():     # if there's no question in survey, show alert
        if len(survey_question) == 0:
            messagebox.showinfo('Start Survey', 'Your survey is empty!')
        else:   # if there are questions, check whether all questions are edited
            unedited = 0
            for question in survey_question:
                if not question['is_edited']:
                    unedited += 1
            if unedited != 0:   # if any question is unedited, show alert
                messagebox.showinfo('Start Survey', f'You have {unedited} unedited question!')
            else:   # if all questions are edited, it's ready to go.
                responder(root)     # create an instance of responder class

    @staticmethod
    def show_result():  # open a window and list current answer records that are ready to export
        if len(answer_record) == 0:     # if there's no record, show alert
            messagebox.showinfo('Show Result', 'No result available!')
        else:   # otherwise, open a popup window, then set up control and position properly
            ...
            for index, record in enumerate(answer_record):  # display answer records
                tk.Label(show_win, text='Record ' + str(index+1) + ': ' + '[ID ' + record['ID'] + ']').grid(sticky='w')

    @staticmethod
    def export_result():    # export current available answer records to csv file
        # sample data: [{'ID': '123', '1': ['aaa'], '2': ['bbb'], '3': [3, 'ccc'], '4': [[0, 1, 0, 0, 1, 1], 'ddd']}]
        # this is a silly algorithm converts answer showed above to more human readable data and write to csv file
        if len(answer_record) > 0:
            # if there are answer records, pick keys of the first record (dict type) as fieldnames for csv file header
            fieldnames = [key for key in answer_record[0]]
            for record in answer_record:
                for item in record:     # loop answers of each question in each record
                    if len(record[item]) == 1 and item != 'ID':  # select answers contain only one element, excluding ID
                        record[item] = str(record[item][0])     # convert answer to string
                    elif len(record[item]) == 2 and item != 'ID':   # select answers contain two element, excluding ID
                        if type(record[item][0]) == list:   # select multiple choice questions
                            results = []    # convert answer [0, 1, 0, 0, 1, 1] to [2, 5, 6]
                            for index, result in enumerate(record[item][0]):
                                if result == 1:
                                    results.append(str(index + 1))
                            record[item][0] = ",".join(results)     # combine answers to a string
                            record[item] = ",".join([str(record[item][0]), str(record[item][1])])
                        else:   # select single choice questions, combine answers to a string
                            record[item] = ",".join([str(record[item][0]), str(record[item][1])])
            # open file dialog and let user to save file
            export_filename = filedialog.asksaveasfile(initialdir='/Users',
                                                       title='Select a directory', filetypes=(('csv files', '*.csv'),
                                                                                              ('all files', '*.*')))
            if export_filename is not None:     # if user didn't hit cancel, returns proper file name
                with open(export_filename.name, 'w') as out_file:
                    w = csv.DictWriter(out_file, fieldnames)    # write csv file from answer record dictionary
                    w.writeheader()
                    for record in answer_record:
                        w.writerow(record)
                answer_record.clear()
        else:   # if there's no answer records, show alert
            messagebox.showinfo('Export Result', 'No exportable answer record!')
    ...
    def update_ques_type(self, *args):  # use self.ques_type to record last choice, and get current choice.
        ...
        if self.option_value.get() != self.ques_type:
            ...
            if self.option_value.get() == 'CO':
                ...
                # hide add button
                self.option_control.hide_add_button()
                # clean up question note
                self.notes_input.delete(0, 'end')
                self.notes_input.insert(0, '')
            # for type 'YN', only display two option cells with text 'Yes' and 'No', 'Reason' check box, hide add button
            elif self.option_value.get() == 'YN':
                # set last choice value
                self.ques_type = self.option_value.get()
...
class responder(tk.Toplevel):
    ...
    def __init__(self, original):
        ...
        # create a list to store user action of current question
        self.user_action = []

    def update_responder(self):     # update appearance of responder
        ...
        # update appearance of page buttons according to current page
        if self.ques_id == 0:   # when it's on the first page
            if len(survey_question) == 1:   # if there's only one question, disable both buttons
            ...
            else:   # otherwise, only disable backward button
                self.backward = tk.Button(self, text='<<', command=self.button_backward, state=tk.DISABLED)
                self.backward.grid(row=1, column=0, sticky='e')
                self.forward = tk.Button(self, text='>>', command=self.button_forward, state=tk.NORMAL)
                self.forward.grid(row=1, column=2, sticky='w')
        elif self.ques_id == len(survey_question) - 1:  # when it's on the last page, disable forward button
            self.forward = tk.Button(self, text='>>', command=self.button_forward, state=tk.DISABLED)
            self.forward.grid(row=1, column=2, sticky='w')
            self.backward = tk.Button(self, text='<<', command=self.button_backward, state=tk.NORMAL)
            self.backward.grid(row=1, column=0, sticky='e')
        else:   # other than above, enable both buttons
            self.forward = tk.Button(self, text='>>', command=self.button_forward, state=tk.NORMAL)
            self.backward = tk.Button(self, text='<<', command=self.button_backward, state=tk.NORMAL)
            self.forward.grid(row=1, column=2, sticky='w')
            self.backward.grid(row=1, column=0, sticky='e')
        ...
        if survey_question[self.ques_id]['question_type'] == 'CO':  # if it's a completion question: only 1 action
            self.user_action.append(answer_input(self, 'Answer:'))  # add an instance of answer_input class
        elif survey_question[self.ques_id]['question_type'] == 'CH':    # if it's a single choice question
            self.user_action.append(answer_CH_LI_YN(self, self.ques_id))  # add an instance of answer_CH_LI_YN class
            if survey_question[self.ques_id]['is_checked'] == '1':  # if other choice is enabled
                self.user_action.append(answer_input(self, 'Other:'))   # add an instance of answer_input class
        elif survey_question[self.ques_id]['question_type'] == 'MC':    # if it's a multiple choice question
            self.user_action.append(answer_MC(self, self.ques_id))  # add an instance of answer_MC class
            if survey_question[self.ques_id]['is_checked'] == '1':  # if other choice is enabled
                self.user_action.append(answer_input(self, 'Other:'))   # add an instance of answer_input class
        elif survey_question[self.ques_id]['question_type'] == 'YN':    # if it's a yes or no question
            self.user_action.append(answer_CH_LI_YN(self, self.ques_id))  # add an instance of answer_CH_LI_YN class
            if survey_question[self.ques_id]['is_checked'] == '1':  # if other choice is enabled
                self.user_action.append(answer_input(self, 'Reason:'))  # add an instance of answer_input class
        elif survey_question[self.ques_id]['question_type'] == 'LI':    # if it's a likert scale question
            self.user_action.append(answer_CH_LI_YN(self, self.ques_id))  # add an instance of answer_CH_LI_YN class
        # display question note on screen
        if survey_question[self.ques_id]['note'] != '':
            question_note(self, self.ques_id)

    def button_forward(self):
        self.save_action()  # save user choice of current question
        self.ques_id += 1   # move to next question
        self.update_responder()     # call method to update appearance of responder

    def button_backward(self):
        self.save_action()  # save user choice of current question
        self.ques_id -= 1   # move to previous question
        self.update_responder()     # call method to update appearance of responder

    def button_exit(self):
        # ask user whether to save answers
        message_win = messagebox.askyesno('Alert', 'Do you want to save the answers before exit?')
        if message_win == 1:    # if yes, call method to save results to answer_record, then close window
            self.save_result()
            self.close_window()
        else:   # if no, close window directly
            self.close_window()

    def save_action(self):
        # save user action to answer list of current user
        if len(self.user_action) == 2:
            # user_action length is 2, meaning user has typed in input box, then save both choice and input
            self.survey_answer[str(self.ques_id + 1)] = [self.user_action[0].get_result(),
                                                         self.user_action[1].get_result()]
            print(f'save action: {self.user_action[0].get_result()}, {self.user_action[1].get_result()}')
        else:   # if there's no input box, get the choice only
            self.survey_answer[str(self.ques_id + 1)] = [self.user_action[0].get_result()]
            print(f'save action: {self.user_action[0].get_result()}')
        print(self.survey_answer)
        # destroy all widgets on screen to clean up onscreen option
        for widget in self.onscreen_widget:
            widget.destroy()
        # save user actions in current question according to question type and input
        self.user_action.clear()  # clean up previous available actions in user_action list
...
# define answer_CH_LI_YN class, for single choice, likert scale and yes-or-no questions
class answer_CH_LI_YN:

    def __init__(self, original, quesid):
        self.button_list = []   # store option buttons of current question
        self.variable = tk.IntVar()     # use tkinter IntVar() variable to store user selection
        if str(quesid + 1) in original.survey_answer:   # if current question has already been answered
            self.variable.set(str(original.survey_answer[str(quesid + 1)][0]))  # load and display previous answer
        for index, option in enumerate(survey_question[quesid]['option']):  # loop through options of current question
            option = textwrap.fill(option, width=40)    # wrap text with 40 characters per line
            self.button_list.append(tk.Radiobutton(original.lower_left, text=option, variable=self.variable,
                                                   value=index + 1))    # add option buttons to button_list
        for index, button in enumerate(self.button_list):
            button.grid(row=index, column=0, sticky='w')    # display option buttons
            original.onscreen_widget.append(button)     # add option button widgets to responder's onscreen_widget

    def get_result(self):   # return current selection
        return self.variable.get()

    def destroy(self):  # destroy all option buttons of current question
        for button in self.button_list:
            button.destroy()


# define answer_MC class, for multiple choice questions
class answer_MC:

    def __init__(self, original, quesid):
        self.button_list = []   # store option buttons of current question
        self.variable_list = []     # store selection of each button
        for index, option in enumerate(survey_question[quesid]['option']):  # loop through options of current question
            var = tk.IntVar()
            self.variable_list.append(var)  # add selection of each button to variable_list
            option = textwrap.fill(option, width=40)    # wrap text with 40 characters per line
            # add option buttons to button_list
        ...
        if str(quesid + 1) in original.survey_answer:   # if current question has already been answered
            for index, option in enumerate(self.button_list):   # load and display previous answer
                if original.survey_answer[str(quesid + 1)][0][index] == 1:
                    option.select()

    def get_result(self):   # return current selection results of each check box
        results = [var.get() for var in self.variable_list]
        return results
...
# define answer_input class, for user input to different types of question
class answer_input:

    def __init__(self, original, label_text):   # create and display widgets of user input
        ...
        original.onscreen_widget.append(self.answer_input)  # add current widgets to responder's onscreen_widget list
        if str(original.ques_id + 1) in original.survey_answer:     # if current question has already been answered
            if survey_question[original.ques_id]['question_type'] == 'CO':  # if it's completion question
                # load the first element in answer record and display
                self.answer_input.insert(0, original.survey_answer[str(original.ques_id + 1)][0])
            else:   # otherwise, load the second element in answer record, which is user input, and display
                self.answer_input.insert(0, original.survey_answer[str(original.ques_id + 1)][1])

    def get_result(self):   # return user input
        if self.answer_input.get() == '':
            result = ''
        else:
            result = self.answer_input.get()
        return result
...
# define question_note class, for displaying question note
class question_note:

    def __init__(self, original, quesid):
        self.note_label = tk.Label(original.lower_right, text='Note:')
        self.note_label.grid(sticky='w')
        label_text = survey_question[quesid]['note']    # load question note from survey_question according to quesid
        ...
        original.onscreen_widget.append(self.note_label)    # add current widgets to responder's onscreen_widget list
        original.onscreen_widget.append(self.note_text)

    def destroy(self):  # destroy note of current question
        self.note_text.destroy()
        self.note_label.destroy()


# define enter_ID class, for asking user to type in ID
class enter_ID:
    ...
    def button_ok(self):
        if self.input_box.get() == '':  # if ID input is empty, show alert
            tk.messagebox.showinfo('Alert', 'Please enter responder ID!')
        else:   # otherwise, save ID as the first element of current answer record, then call methods to update screen
